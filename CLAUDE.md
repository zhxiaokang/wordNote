...
- Vocab tab grouping (`getGroupedVocab(mode)`) groups by `createdDate` into week (Mon–Sun, via `getWeekRange` in `utils/date.js`) / month / year buckets, newest-first, empty buckets simply never created.
  - `pages/vocab/index.js` keeps per-mode expand/collapse state (`expandedByMode`, keyed by group `key`) and scroll position (`scrollTopByMode`, from `onPageScroll`) as page-instance properties, not `data` — they must survive `onShow` (tab switches, returning from vocab-detail) without a fresh render resetting them. `loadGroups()` re-reads word data from `wordStore` every `onShow` (so edits made in vocab-detail show up) but only defaults a group's `expanded` flag (index 0 = expanded) the first time its `key` is seen; an already-seen key keeps the user's last toggle. Don't go back to unconditionally rebuilding `expanded` by index on every load — that's what caused the list to snap back to its default expand/collapse state and scroll-to-top every time the user left and returned to 词汇.
